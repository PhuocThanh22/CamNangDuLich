...
function ReviewTab({ reviews, avgRating }: { reviews: ReviewItem[]; avgRating: string }) {
  return (
    <motion.div
      ...
      className="space-y-5"
    >
      <div className="rounded-2xl bg-white p-6 shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]">
        <div className="mb-5 flex items-center gap-3">
          <h2 className="text-[17px] font-bold text-slate-900 dark:text-white">Đánh giá từ cộng đồng</h2>
          <span className="rounded-lg bg-blue-600 px-2 py-0.5 text-[13px] font-bold text-white">{avgRating}</span>
        </div>
        {reviews.length > 0 ? (
          <div className="space-y-4">
            {reviews.map((review) => (
              <div key={review.id} className="rounded-xl border border-slate-100 p-4 dark:border-slate-700">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-[14px] font-semibold text-slate-800 dark:text-white">{review.nguoidung_ten || 'Ẩn danh'}</p>
                      <p className="text-[12px] text-slate-400 dark:text-slate-500">{review.created_at ? new Date(review.created_at).toLocaleDateString('vi-VN') : ''}</p>
                    </div>
                  </div>
                  <div className="flex shrink-0 text-amber-400">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star key={s} className={`h-4 w-4 ${s <= Math.round(review.diemdanhgia) ? 'fill-current' : 'text-slate-200 dark:text-slate-600'}`} />
                    ))}
                  </div>
                </div>
                {review.noidung && (
                  <p className="text-[13px] leading-relaxed text-slate-600 dark:text-slate-300">{review.noidung}</p>
                )}
              </div>
            ))}
          </div>
        ) : (
          <p className="py-8 text-center text-[14px] text-slate-400 dark:text-slate-500">Chưa có đánh giá nào. Hãy là người đầu tiên!</p>
        )}
      </div>
    </motion.div>
  );
...
function StarRating({ value }: { value: number }) {
  return (
    <div className="flex text-amber-400">
      {[1, 2, 3, 4, 5].map((s) => (
        <Star key={s} className={`h-3.5 w-3.5 ${s <= Math.round(value) ? 'fill-current' : 'text-slate-200 dark:text-slate-600'}`} />
      ))}
    </div>
  );
}
...
export default function PlacePage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f8fafc] dark:bg-[#0b1120]">
        <div className="text-center">
          ...
          <p className="text-[14px] text-slate-500 dark:text-slate-400">Đang tải...</p>
        </div>
      </div>
    );
  }
  ...
  if (!item) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f8fafc] dark:bg-[#0b1120]">
        <div className="text-center">
          <p className="text-[16px] font-semibold text-slate-600 dark:text-slate-300">Không tìm thấy địa điểm</p>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0b1120]">
      ...
      <div className="bg-white border-b border-slate-100 shadow-sm dark:bg-[#111a2e] dark:border-slate-700">
        <div className="mx-auto max-w-5xl px-5 py-5 sm:px-8 lg:px-10">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex-1">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <span className="flex items-center gap-1.5 rounded-full bg-blue-100 px-3 py-1 text-[12px] font-semibold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                ...
                {item.daduyet === false && (
                  <span className="rounded-full bg-amber-100 px-3 py-1 text-[12px] font-semibold text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">
                    ● Chờ duyệt
                  </span>
                )}
              </div>

              <h1 className="mb-2 text-[24px] font-black text-slate-900 sm:text-[28px] dark:text-white">{item.ten}</h1>

              <div className="flex flex-wrap items-center gap-4 text-[13px] text-slate-600 dark:text-slate-300">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-slate-400 dark:text-slate-500">({reviews.length || item?.luotdanhgia || '0 đánh giá'})</span>
                </div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-[13px] font-semibold transition ${
                    isFavorite
                      ? 'border-red-200 bg-red-50 text-red-600 dark:border-red-900/40 dark:bg-red-900/40 dark:text-red-400'
                      : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:bg-[#0f172a] dark:text-slate-300 dark:hover:bg-slate-800'
                  }`}
                >
      ...
      <div className="sticky top-[60px] z-40 border-b border-slate-100 bg-white shadow-sm dark:border-slate-700 dark:bg-[#111a2e]">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`relative mr-6 shrink-0 whitespace-nowrap py-4 text-[14px] font-semibold transition-colors ${
                activeTab === tab ? 'text-[#3b82f6]' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white'
              }`}
            >
              {tab}
              {activeTab === tab && (
          ...
          <div className="space-y-5 lg:col-span-2">
            <AnimatePresence mode="wait">
              {activeTab === 'Tổng quan' && (
                ...
                >
                  <div className="rounded-2xl bg-white p-6 shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]">
                    <h2 className="mb-3 text-[17px] font-bold text-slate-900 dark:text-white">Giới thiệu</h2>
                  ...
                  {(item.monan || item.tienich || item.trangweb) && (
                    <div className="rounded-2xl bg-white p-6 shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]">
                      <h2 className="mb-4 text-[16px] font-bold text-slate-900 dark:text-white">Thông tin chi tiết</h2>
                      <div className="grid gap-4 sm:grid-cols-2">
                        {item.monan && (
                          <div>
                            <p className="mb-1 text-[12px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Món đặc trưng</p>
                            <div className="flex flex-wrap gap-1.5">
                              {item.monan.split(',').map((m, i) => (
                                <span key={i} className="rounded-lg bg-orange-50 px-2.5 py-1 text-[13px] font-medium text-orange-700 dark:bg-orange-900/40 dark:text-orange-300">{m.trim()}</span>
                              ))}
                            </div>
                          </div>
                        )}
                        {item.tienich && (
                          <div>
                            <p className="mb-1 text-[12px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Tiện ích</p>
                            <div className="flex flex-wrap gap-1.5">
                              {item.tienich.split(',').map((t, i) => (
                                <span key={i} className="rounded-lg bg-green-50 px-2.5 py-1 text-[13px] font-medium text-green-700 dark:bg-green-900/40 dark:text-green-300">{t.trim()}</span>
                              ))}
                            </div>
                          </div>
                        )}
                        {item.trangweb && (
                          <div>
                            <p className="mb-1 text-[12px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Trang web</p>
                            <a href={item.trangweb} target="_blank" rel="noopener noreferrer" className="text-[14px] text-blue-600 hover:underline dark:text-blue-400">
                  ...
                  <div className="rounded-2xl bg-white p-6 shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]">
                    ...
                    {item.giohoatdong ? (
                      <p className="text-[14px] text-slate-600 dark:text-slate-300">{item.giohoatdong}</p>
                    ) : (
                      <div className="space-y-2">
                        {HOURS.map((h) => (
                          <div key={h.day} className="flex items-center justify-between text-[14px]">
                            <span className="text-slate-600 dark:text-slate-300">{h.day}</span>
                            <span className="font-semibold text-slate-800 dark:text-white">{h.time}</span>
                          </div>
                        ))}
                      </div>
                    )}
...
<p className="text-[13px] leading-relaxed text-slate-600 dark:text-slate-300">{review.noidung}</p>
                      ...
                      {reviews.length > 3 && (
                        <button
                          ...
                          className="mt-4 w-full rounded-xl border border-blue-200 bg-blue-50 py-2.5 text-[13px] font-bold text-blue-700 transition hover:bg-blue-100 dark:border-blue-900/40 dark:bg-blue-900/40 dark:text-blue-300 dark:hover:bg-blue-900/60"
                        >
                ...
                >
                  <div className="rounded-2xl bg-white p-6 shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]">
                    <div className="mb-5 flex items-center gap-2">
                      ...
                      <h2 className="text-[17px] font-bold text-slate-900 dark:text-white">Thực đơn</h2>
                      {menuItems.length > 0 && (
                        <span className="rounded-lg bg-orange-100 px-2.5 py-0.5 text-[12px] font-semibold text-orange-700 dark:bg-orange-900/40 dark:text-orange-300">
                    ...
                    {menuItems.length > 0 ? (
                          ...
                          <div key={menuItem.id} className="flex gap-4 py-4 first:pt-0 last:pb-0">
                            ...
                            <div className="flex-1">
                              <div className="mb-1 flex items-start justify-between gap-2">
                                <h3 className="text-[14px] font-semibold text-slate-800 dark:text-white">{menuItem.ten}</h3>
                                {menuItem.gia && (
                                  <span className="shrink-0 text-[14px] font-bold text-green-700 dark:text-green-400">{menuItem.gia}</span>
                                )}
                              </div>
                              {menuItem.mota && (
                                <p className="text-[13px] text-slate-500 dark:text-slate-400">{menuItem.mota}</p>
                              )}
                            </div>
                            {isOwner && (
                              <button
                                ...
                                className="shrink-0 self-center text-slate-300 transition hover:text-red-500 dark:text-slate-500 dark:hover:text-red-400"
                              >
                      ...
                      <div className="flex flex-col items-center py-8 text-center">
                        <UtensilsCrossed className="mb-3 h-10 w-10 text-slate-300 dark:text-slate-500" />
                        <p className="text-[14px] text-slate-400 dark:text-slate-500">Chưa có thực đơn</p>
                      </div>
                    )}
                  </div>
                </motion.div>
                ...
                >
                  <div className="rounded-2xl bg-white p-6 shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]">
                    <div className="mb-5 flex items-center gap-2">
                      ...
                      <h2 className="text-[17px] font-bold text-slate-900 dark:text-white">Thư viện ảnh</h2>
                      {galleryImages.length > 0 && (
                        <span className="ml-auto rounded-lg bg-blue-100 px-2.5 py-0.5 text-[12px] font-semibold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                    ...
                    {galleryImages.length > 0 ? (
                      ...
                      <div className="flex flex-col items-center py-8 text-center">
                        <Image className="mb-3 h-10 w-10 text-slate-300 dark:text-slate-500" />
                        <p className="text-[14px] text-slate-400 dark:text-slate-500">Chưa có ảnh</p>
                      </div>
                    )}
                  </div>
                </motion.div>
              ...
              {activeTab !== 'Tổng quan' && activeTab !== 'Thực đơn' && !activeTab.startsWith('Đánh giá') && activeTab !== 'Ảnh' && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="flex flex-col items-center rounded-2xl bg-white p-12 text-center shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]"
                >
                  <Construction className="mb-4 h-12 w-12 text-slate-300 dark:text-slate-500" />
                  <h3 className="mb-2 text-[16px] font-bold text-slate-800 dark:text-white">Đang cập nhật</h3>
                  <p className="text-[14px] text-slate-500 dark:text-slate-400">Nội dung &quot;{activeTab}&quot; sẽ sớm được thêm vào.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          <div className="space-y-5">
            <div className="rounded-2xl bg-white p-5 shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]">
              <h3 className="mb-4 text-[15px] font-bold text-slate-900 dark:text-white">Thông tin địa điểm</h3>
              <ul className="space-y-3">
                <li className="flex items-start gap-2.5 text-[13px] text-slate-600 dark:text-slate-300">
                  <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-blue-500" />
                  <span className="leading-relaxed">{item.diachi || '123 Hàng Bồ, phường Hàng Bồ, quận Hoàn Kiếm, Hà Nội'}</span>
                </li>
                <li className="flex items-center gap-2.5 text-[13px] text-slate-600 dark:text-slate-300">
            ...
            <div className="overflow-hidden rounded-2xl bg-white shadow-[0_2px_16px_rgba(0,0,0,0.05)] dark:bg-[#111a2e]">
        ...
        {showMenuForm && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-[#111a2e]"
            >
              <h3 className="mb-5 text-[17px] font-bold text-slate-900 dark:text-white">Thêm món mới</h3>
              <div className="space-y-4">
                <div>
                  <label className="mb-1 block text-[13px] font-semibold text-slate-600 dark:text-slate-300">Tên món *</label>
                  <input
                    type="text"
                    value={menuFormData.ten}
                    onChange={(e) => setMenuFormData({ ...menuFormData, ten: e.target.value })}
                    placeholder="VD: Phở bò tái"
                    className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-[14px] outline-none transition focus:border-blue-400 dark:bg-[#0f172a] dark:border-slate-700 dark:text-slate-200"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[13px] font-semibold text-slate-600 dark:text-slate-300">Giá</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="mb-1 block text-[13px] font-semibold text-slate-600 dark:text-slate-300">Mô tả</label>
                  <textarea
                    value={menuFormData.mota}
                    onChange={(e) => setMenuFormData({ ...menuFormData, mota: e.target.value })}
                    placeholder="Mô tả ngắn về món ăn"
                    rows={3}
                    className="w-full resize-none rounded-xl border border-slate-200 px-4 py-2.5 text-[14px] outline-none transition focus:border-blue-400 dark:bg-[#0f172a] dark:border-slate-700 dark:text-slate-200"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[13px] font-semibold text-slate-600 dark:text-slate-300">URL ảnh</label>
                  <input
                    type="text"
              ...
              <div className="mt-6 flex gap-3">
                <button
                  type="button"
                  onClick={() => setShowMenuForm(false)}
                  className="flex-1 rounded-xl border border-slate-200 py-2.5 text-[13px] font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                >
                  Hủy
                </button>
                <button
